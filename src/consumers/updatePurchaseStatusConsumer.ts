...
const updatePurchaseStatusConsumer = async (channel: amqp.Channel) => {
  try {
    channel.assertExchange(
      MESSAGE_KEY.SABAY_ONE_USER_PURCHASE_UPDATE,
      QUEUE_TYPE.FANOUT,
      { durable: true }
    );

    const assertQueue = await channel.assertQueue(
      EXCHANGE_QUEUE_MESSAGE_KEY.SABAY_ONE_USER_PURCHASE_UPDATE_QUEUE,
      { exclusive: false }
    );
...
export default updatePurchaseStatusConsumer;
